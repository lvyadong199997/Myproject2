...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="../admin/order-search" method="post" id="searchForm">
    ...
    <div class="page">
        <div>
            <a class="prev"
               onclick="prePage()" href="javascript:;">&lt;&lt;</a>
            <c:forEach begin="1" end="${pageResult.totalPage}" var="i">
                ...
                <%--不是当前页--%>
                <c:if test="${i!=pageResult.currPage}">
                    <a class="num" href="javascript:;" onclick="page(${i})">${i}</a>
                </c:if>
            </c:forEach>
            <a class="next"
               onclick="nextPage()" href="javascript:;">&gt;&gt;</a>
        </div>
...
<script>
    var pre = "../admin/order-search?pageNum=${pageResult.currPage-1==0?1:pageResult.currPage-1}";
    var next = "../admin/order-search?pageNum=${pageResult.currPage+1>pageResult.totalPage?pageResult.currPage:pageResult.currPage+1}";
    var p = "../admin/order-search?pageNum=";

    function getKey() {
        var searchMap =${jsonMap};
        var key = "";
        for (var i in searchMap) {
            key += (i + "=" + (searchMap[i] == null ? "" : searchMap[i]) + "&");
        }
    ...
    function putForm() {
        $("#payMethod").sibling().change(function (e) {
            $("#searchForm").submit();
        })
